        ...
        <%@ page import="Models.BorrowRecordDTO" %>
            ...
            <div class="page-header"
                ...
                <div class="page-title">
                    <h1 style="font-size: 2.25rem; font-weight: 800; color: #1c1e21;">Loan Management</h1>
                    <p style="color: #65676b; font-size: 1.125rem;">Monitor and manage all active and past book loans.
                    </p>
                </div>
                <div style="display: flex; gap: 12px;">
                    <a href="Borrow?action=quickReturn" class="btn-primary"
                        style="padding: 12px 24px; border-radius: 12px; font-weight: 700; background: #15803d; box-shadow: 0 4px 12px rgba(21, 128, 61, 0.2); text-decoration: none; display: flex; align-items: center; gap: 8px; color: white;">
                        <span class="material-symbols-outlined">assignment_return</span>
                        <span>Return Book</span>
                    </a>
                    <a href="Borrow?action=walkin" class="btn-primary"
                        style="padding: 12px 24px; border-radius: 12px; font-weight: 700; background: var(--primary-gradient); box-shadow: 0 4px 12px rgba(24, 119, 242, 0.2); text-decoration: none; display: flex; align-items: center; gap: 8px; color: white;">
            ...
            <div class="card"
                ...
                <table class="data-table" id="loanTable"
                    ...
                    <tbody id="loanTableBody">
                        <% List<BorrowRecordDTO> listRecords = (List<BorrowRecordDTO>)
                                request.getAttribute("listRecords");
                                if (listRecords != null && !listRecords.isEmpty()) {
                                for (BorrowRecordDTO record : listRecords) {
                                String status = record.getStatus();
                                String bg = "#f1f5f9", cl = "#475569";
                                if ("BORROWED".equalsIgnoreCase(status) && record.getDue_date() != null) {
                                    java.time.LocalDate dueDateLocal = record.getDue_date().toLocalDate();
                                    if (java.time.LocalDate.now().isAfter(dueDateLocal)) {
                                        status = "OVERDUE";
                                    }
                                }
                                ...
                                String badgeStyle = "background: " + bg + "; color: " + cl + ";";
                                String borrowDate = record.getPickup_date() != null ? record.getPickup_date().toString()
                                : "";
                                String dueDate = record.getDue_date() != null ? record.getDue_date().toString() : "";
                                String returnDate = record.getReturn_date() != null ? record.getReturn_date().toString()
                                : "";
                                %>
                                <tr class="loan-row" data-record-id="<%= record.getBorrow_id() %>"
                                    ...
                                    data-borrow-date="<%= borrowDate %>" data-due-date="<%= dueDate %>"
                                    data-return-date="<%= returnDate %>" style="transition: all 0.2s;"
                                    ...
                                    <td style="padding: 20px 24px; border-bottom: 1px solid #f1f5f9;">
                                        ...
                                        <div style="font-size: 0.75rem; color: #65676b;">ID: <%= record.getMember_id()
                                                %>
                                        </div>
                                    </td>
                                    <td style="padding: 20px 24px; border-bottom: 1px solid #f1f5f9;">
                                        ...
                                        <div style="font-size: 0.75rem; color: #65676b;">ID: <%= record.getBook_id() %>
                                        </div>
                                    </td>
                                    <td style="padding: 20px 24px; border-bottom: 1px solid #f1f5f9; color: #4b5563;">
                                        <%= borrowDate.isEmpty() ? "---" : borrowDate %>
                                    </td>
                                    <td style="padding: 20px 24px; border-bottom: 1px solid #f1f5f9; color: #4b5563;">
                                        <%= dueDate.isEmpty() ? "---" : dueDate %>
                                    </td>
                                    <td style="padding: 20px 24px; border-bottom: 1px solid #f1f5f9; color: #4b5563;">
                                        <%= returnDate.isEmpty() ? "---" : returnDate %>
                                    </td>
                                    <td style="padding: 20px 24px; border-bottom: 1px solid #f1f5f9;">
                                        <span
                                        ...
                                        <div style="display: flex; justify-content: flex-end; gap: 8px;">
                                            <% if ("APPROVED".equalsIgnoreCase(status)) { %>
                                                <a href="Borrow?action=showConfirmPickup&id=<%= record.getBorrow_id() %>"
                                                    class="btn-primary"
                                                    style="padding: 6px 12px; font-size: 0.75rem; border-radius: 8px;">Confirm
                                                    Pickup</a>
                                                <% } else if ("BORROWED".equalsIgnoreCase(status) || "OVERDUE".equalsIgnoreCase(status)) { %>
                                                    <a href="Borrow?action=quickReturn&id=<%= record.getBorrow_id() %>"
                                                        class="btn-primary"
                                                        style="padding: 6px 12px; font-size: 0.75rem; border-radius: 8px; background: #15803d;">Return
                                                        Book</a>
                                                    <% } %>
                                        </div>
                                    </td>
                                ...
                                <% } } else { %>
                                    <tr id="emptyRow">
                                        <td colspan="8" style="text-align: center; padding: 60px; color: #65676b;">
                                            <span class="material-symbols-outlined"
                                                style="font-size: 48px; color: #cbd5e1;">receipt_long</span>
                                            <p style="margin-top: 16px;">No loan records found.</p>
                                        </td>
                                    </tr>
                                    <% } %>
                    </tbody>
            ...
            <!-- No Results Row (hidden, shown by JS when filters match nothing) -->
            <script>
                ...
                function applyFilters() {
                    ...
                    var hasActiveFilter = query || statusVal || borrowDateVal || dueDateVal || returnDateVal;
                    ...
                    rows.forEach(function (row) {
                        ...
                        if (matchSearch && matchStatus && matchBorrow && matchDue && matchReturn) {
                            row.style.display = '';
                            visibleCount++;
                        } else {
                    ...
                    if (visibleCount === 0 && rows.length > 0) {
                        if (!emptyRow) {
                            ...
                            emptyRow.innerHTML = '<td colspan="8" style="text-align: center; padding: 60px; color: #65676b;"><span class="material-symbols-outlined" style="font-size: 48px; color: #cbd5e1;">search_off</span><p style="margin-top: 16px;">No records match your search or filters.</p></td>';
                ...
                // Run on load if there's any pre-filled value or URL parameter
